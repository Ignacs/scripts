...
if [ $# -gt 0 ]; then
	...
	# make ASP_MODEL=$TARGET ASP_PROFILE=${PROFILE} ASP_MAX_JOBS=AUTO -C app/private/wdwdv3 qcom_related_install qcom_ezmesh_install qcom_related &> $OUTPUT_FILE &&
	# make ASP_MODEL=$TARGET ASP_PROFILE=${PROFILE} ASP_MAX_JOBS=AUTO -C app/private/wdwdv3 qcom_related &> $OUTPUT_FILE &&
	# make ASP_MODEL=$TARGET ASP_PROFILE=${PROFILE} ASP_MAX_JOBS=AUTO -C app/private/wdwdv3 qcom_related_install ezmesh_install &> $OUTPUT_FILE &&
	make ASP_MODEL=$TARGET ASP_PROFILE=${PROFILE} ASP_MAX_JOBS=AUTO -C app/private/wdwdv3 qcom_related qcom_related_install &> $OUTPUT_FILE &&
