...
function usage()
{
	echo " !! Usage: $1 <-s F/B> <-r Redmine number> <-t Redmine title> <-m commit_message file> <-l commit list> <-a(--amend)>"
	echo ""
}
if [ $# -lt 6 ]; then 
...
while getopts "s:t:r:m:l:a" flag
do 
    case $flag in 
		a)
			AMEND=1
...
# Amy 2018/07/18 : GIT format 
# F/B#(RM ID)-(RM title)
if [ ! -f "$CI_MESG" ] ; then 
    echo "Use default file: git_commit.txt"
	CI_MESG=git_commit.txt
	CI_MESG_WRITING=git_commit_writing.txt
    ...
    if [ -z "$CI_MESG" ] ; then 
		...
		sed -e "1c$ST#"$RMN"-"$RMTITLE $CI_MESG > git_commit.txt
	fi
	CI_MESG=git_commit.txt
fi

if [ ! -f "$CI_LIST" ] ; then 
    echo "Use default file 'git_ci'"
    CI_LIST="git_ci"
else 
...
if [ "y" = "$TEST_CASE" ] ; then 
	...
	if [ 1 -eq  $AMEND ] ; then 
		echo "!!!!!!!!!!!!! amend !!!!!!!!!!!!!!!!!!"
		git ci -F $CI_MESG `cat $CI_LIST` --amend
	else
		echo "commit ..."
		git ci -F $CI_MESG `cat $CI_LIST` 
	fi
